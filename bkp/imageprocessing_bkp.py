...
def choiceFace(filename):
    """ Face Detection """
    for file in filename:
        print("Starting process ----")
        print(file)
        faceImg = readImageRGB(file)
        ...
        writeimage('output/face/', 'C_' + file.split('/')[-1][:-4], faceImg_original)
        writeimage('output/face/', 'D_' + file.split('/')[-1][:-4], faceImg_distributed)
        plt.show()
